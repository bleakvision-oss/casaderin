...
function bind(){ document.querySelectorAll('.nav-link').forEach((b)=>b.onclick=()=>{setActivePanel(b.dataset.panel);render();}); const mobileNav=document.getElementById('mobileNav'); if(mobileNav){mobileNav.value=activePanel; mobileNav.onchange=()=>{setActivePanel(mobileNav.value);render();};}
 ...
 const dnevnikForm=document.getElementById('dnevnikForm'); if(dnevnikForm){if(dnevnikEditingId){const activeEntry=(data.dailyEntries||[]).find((x)=>x.id===dnevnikEditingId);if(activeEntry){dnevnikForm.date.value=activeEntry.date||'';dnevnikForm.dishName.value=activeEntry.dishName||'';dnevnikForm.preparedQty.value=activeEntry.preparedQty??0;dnevnikForm.soldQty.value=activeEntry.soldQty??0;dnevnikForm.leftoverQty.value=activeEntry.leftoverQty??0;dnevnikForm.resultStatus.value=activeEntry.resultStatus||'okej';dnevnikForm.notes.value=activeEntry.notes||'';}}document.getElementById('saveDnevnikEntry')?.addEventListener('click',()=>{const form=document.getElementById('dnevnikForm');console.log('saveDnevnikEntry clicked');console.log('form found',!!form);if(!form||!form.reportValidity())return;snapshotData();console.log('dailyEntries before',data.dailyEntries);const entryId=String(dnevnikEditingId||'').trim()||crypto.randomUUID();const next={id:entryId,date:String(form.date?.value||''),dishName:String(form.dishName?.value||''),preparedQty:Number(form.preparedQty?.value||0),soldQty:Number(form.soldQty?.value||0),leftoverQty:Number(form.leftoverQty?.value||0),resultStatus:String(form.resultStatus?.value||'okej'),notes:String(form.notes?.value||'')};data.dailyEntries=(data.dailyEntries||[]).filter((x)=>x.id!==next.id);data.dailyEntries.push(next);console.log('dailyEntries after',data.dailyEntries);dnevnikEditingId='';setActivePanel('dnevnik');markDirty();render();});}
