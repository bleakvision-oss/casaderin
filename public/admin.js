...
function allowedPanels(){const staticPanels=['daily','lunch','weekly','desserts','settings','export','qr','analytics','dnevnik'];const drinkPanels=(data?.drinkCategories||[]).map((_,i)=>`drinks-${i}`);return new Set([...staticPanels,...drinkPanels]);}
...
return panelShell('dnevnik','Dnevnik','Interni dnevnik priprave in prodaje jedi.',`
  ...
  <section class='dashboard-card dnevnik-section'>
    <h3>Vnosi</h3>
    <div class='item-list dnevnik-entry-list'>${filteredEntries.map((entry)=>`<article class='admin-item dnevnik-entry'><div class='item-shell'><div class='admin-item-card status-available'><div class='item-main'><h3 class='item-title'>${entry.dishName||'—'}</h3><p class='item-description'>Datum: ${entry.date||'—'} · Pripravljeno: ${entry.preparedQty??0} · Prodano: ${entry.soldQty??0} · Ostalo: ${entry.leftoverQty??0}</p><p class='item-description'>Status: ${entry.resultStatus||'—'}${entry.notes?` · Opombe: ${entry.notes}`:''}</p></div><div class='item-actions dnevnik-actions'><button class='btn edit-dnevnik' data-entry-id='${entry.id}' type='button'>Uredi</button><button class='btn del-dnevnik' data-entry-id='${entry.id}' type='button'>Izbriši</button></div></div></div></article>`).join('')}</div>
  </section>`,false,false);} 
...
function panelShell(key,title,desc,body,allowStatus,canDup,drinkIdx=null){return `<div class='panel-shell'><div class='panel-head'><div><h2>${title}</h2><p>${desc}</p></div><div class='inline'><a class='btn' href='/menu.html' target='_blank'>Predogled menija</a>${key!=='analytics'&&key!=='settings'&&key!=='export'&&key!=='qr'&&key!=='dnevnik'?`<button class='btn add' data-add-section='${key}'>Dodaj</button>`:''}${canDup!==false?`<button class='btn duplicate-last' data-dup-section='${key}' ${canDup?'':'disabled'}>Dupliciraj zadnjo</button>`:''}</div></div>${body}</div>`;}
...
function bind(){ document.querySelectorAll('.nav-link').forEach((b)=>b.onclick=()=>{setActivePanel(b.dataset.panel);render();}); const mobileNav=document.getElementById('mobileNav'); if(mobileNav){mobileNav.value=activePanel; mobileNav.onchange=()=>{setActivePanel(mobileNav.value);render();};}
 ...
 const dnevnikForm=document.getElementById('dnevnikForm'); if(dnevnikForm){if(dnevnikEditingId){const activeEntry=(data.dailyEntries||[]).find((x)=>x.id===dnevnikEditingId);if(activeEntry){dnevnikForm.date.value=activeEntry.date||'';dnevnikForm.dishName.value=activeEntry.dishName||'';dnevnikForm.preparedQty.value=activeEntry.preparedQty??0;dnevnikForm.soldQty.value=activeEntry.soldQty??0;dnevnikForm.leftoverQty.value=activeEntry.leftoverQty??0;dnevnikForm.resultStatus.value=activeEntry.resultStatus||'okej';dnevnikForm.notes.value=activeEntry.notes||'';}}const upsertDnevnikEntry=(event)=>{if(event){event.preventDefault();event.stopPropagation();}if(!dnevnikForm.reportValidity())return;snapshotData();const fd=new FormData(dnevnikForm);const editingId=dnevnikEditingId;const next={id:editingId||crypto.randomUUID(),date:String(fd.get('date')||''),dishName:String(fd.get('dishName')||''),preparedQty:Number(fd.get('preparedQty')||0),soldQty:Number(fd.get('soldQty')||0),leftoverQty:Number(fd.get('leftoverQty')||0),resultStatus:String(fd.get('resultStatus')||'okej'),notes:String(fd.get('notes')||'')};const idx=(data.dailyEntries||[]).findIndex((x)=>x.id===editingId);if(idx>=0)data.dailyEntries[idx]=next;else data.dailyEntries.push(next);dnevnikEditingId='';markDirty();render();};dnevnikForm.addEventListener('submit',upsertDnevnikEntry);const saveDnevnikEntry=document.getElementById('saveDnevnikEntry');if(saveDnevnikEntry){saveDnevnikEntry.onclick=upsertDnevnikEntry;}}
 const dnevnikFilterForm=document.getElementById('dnevnikFilterForm'); if(dnevnikFilterForm){dnevnikFilterForm.onsubmit=(e)=>{e.preventDefault();const fd=new FormData(dnevnikFilterForm);dnevnikDateFilter=String(fd.get('filterDate')||'');render();};}
 ...
 document.querySelectorAll('.edit-dnevnik').forEach((b)=>b.onclick=()=>{const entry=(data.dailyEntries||[]).find((x)=>x.id===b.dataset.entryId);if(!entry)return;dnevnikEditingId=entry.id;render();});
 document.querySelectorAll('.del-dnevnik').forEach((b)=>b.onclick=()=>{snapshotData();const id=b.dataset.entryId;data.dailyEntries=(data.dailyEntries||[]).filter((x)=>x.id!==id);if(dnevnikEditingId===id)dnevnikEditingId='';markDirty();render();});
