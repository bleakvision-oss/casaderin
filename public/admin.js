...
function itemCard(section, i, x, drinkCatIdx=null){
  const isDrink = drinkCatIdx !== null;
  return `<article class='admin-item status-${x.status||'available'}' data-s='${section}' data-i='${i}' data-c='${drinkCatIdx??''}'>
    <div class='item-shell'>
      <div class='item-top'>
        <div class='item-left'>
          <button type='button' class='drag-handle up' aria-label='Premakni gor'>⋮⋮</button>
          <div><h4>${t(x.name) || '—'}</h4><p class='drink-desc'>${t(x.description) || 'Brez opisa'}</p></div>
        </div>
        <div class='item-right'>
          <label class='compact-field'>€<input data-f='price' value='${x.price||''}'></label>
          ${isDrink?'':statusPills(x)}
          <button class='btn duplicate-item'>Dupliciraj</button>
          <button class='btn del'>Izbriši</button>
          <button type='button' class='btn down'>↓</button>
        </div>
      </div>
      <fieldset><legend>${t(data.translations.allergens)}</legend>${allergenPills(x)}</fieldset>
      <details class='advanced'><summary>Prevodi in napredne nastavitve</summary>
        ${langFields(x,'name')}${langFields(x,'description')}
        <div class='inline'><button class='btn suggest'>Predlagaj prevode</button></div>
        <div class='row'><label>Type<input data-f='type' value='${x.type||''}'></label><label>Cena<input data-f='price' value='${x.price||''}'></label><label>Sort<input data-f='sortOrder' value='${x.sortOrder||0}'></label></div>
      </details>
    </div>
  </article>`;
}
function section(key,titleKey){const arr=data.sections[key]||[];return `<section class='dashboard-card' data-section='${key}'><div class='section-header'><h3>${t(data.translations[titleKey])}</h3><div class='inline action-buttons'><button class='btn add' data-add='${key}'>+ ${t(data.translations.add)}</button><button class='btn duplicate-last' data-dup='${key}' ${arr.length?'':'disabled'}>Dupliciraj zadnjo</button></div></div>${arr.map((x,i)=>itemCard(key,i,x)).join('')}</section>`;}
function drinksSection(){return `<section class='dashboard-card'><div class='section-header'><h3>${t(data.translations.beverage)}</h3></div>${(data.drinkCategories||[]).map((cat,ci)=>`<div class='drink-group'><h4>${t(cat.title)}</h4>${(cat.items||[]).map((x,i)=>itemCard('drinks',i,x,ci)).join('')}<div class='inline action-buttons'><button class='btn add-drink' data-ci='${ci}'>+ ${t(data.translations.add)}</button><button class='btn duplicate-last-drink' data-dup-ci='${ci}' ${(cat.items||[]).length?'':'disabled'}>Dupliciraj zadnjo</button></div></div>`).join('')}</section>`;}
function settingsCard(){const s=data.settings;return `<section class='dashboard-card' id='restaurant-settings'><div class='section-header'><h3>Nastavitve</h3></div><div class='row'><label>Restaurant name<input data-settings='restaurantName' value='${s.restaurantName||''}'></label><label>Phone number<input data-settings='phone' value='${s.phone||''}'></label><label>Address<input data-settings='address' value='${s.address||''}'></label></div><label>Google Maps URL<input data-settings='googleMapsUrl' value='${s.googleMapsUrl||''}'></label><label>Instagram URL<input data-settings='instagramUrl' value='${s.instagramUrl||''}'></label>${langFields(s,'openingHours')}${langFields(s,'footerText')}<label>Optional hero image URL<input data-settings='heroImageUrl' value='${s.heroImageUrl||''}'></label></section>`;}
...
function analyticsCard(){const a=data.analyticsSummary||{total:0,byLanguage:{},byDevice:{mobile:0,desktop:0},today:0,last7Days:0};return `<section class='dashboard-card'><div class='section-header'><h3>Analytics</h3></div><div class='analytics-grid'><div><strong>Skupaj ogledov:</strong> ${a.total||0}</div><div><strong>Ogledi danes:</strong> ${a.today||0}</div><div><strong>Zadnjih 7 dni:</strong> ${a.last7Days||0}</div><div><strong>Jeziki:</strong> SL ${a.byLanguage?.sl||0} · EN ${a.byLanguage?.en||0} · IT ${a.byLanguage?.it||0} · DE ${a.byLanguage?.de||0}</div></div></section>`;}
function sidebar(){return `<aside class='sidebar'><div class='brand-mini'>Casa de Rin</div><nav><h5>Meni</h5><a href='#daily'>Dnevna ponudba</a><a href='#lunch'>Kosilo</a><a href='#weekly'>Tedenska ponudba</a><a href='#desserts'>Sladice</a><h5>Pijača</h5><a href='#drinks'>Kategorije pijač</a><h5>Sistem</h5><a href='#restaurant-settings'>Nastavitve</a><a href='#analytics'>Analytics</a><button class='btn' id='exportBackup'>Izvoz / Uvoz</button></nav></aside>`;}
function render(){document.getElementById('app').innerHTML=`<div class='admin-shell'>${sidebar()}<div class='admin-main'><header class='top-head'><div><h2>Administracija menija</h2><p>Urejanje vsebine z boljšim pregledom in hitrejšim delom.</p></div><div class='inline'><button class='btn' id='logoutBtn'>Odjava</button><button class='btn add' data-add='daily'>Dodaj</button><a class='btn' href='/menu.html' target='_blank'>Predogled menija</a></div></header><div class='admin-content' id='daily'>${analyticsCard().replace("<section class='dashboard-card'>","<section class='dashboard-card' id='analytics'>")}${settingsCard()}${section('daily','section_daily').replace("<section class='dashboard-card'","<section class='dashboard-card' id='daily'")}${section('lunch','lunch').replace("<section class='dashboard-card'","<section class='dashboard-card' id='lunch'")}${section('weekly','section_weekly').replace("<section class='dashboard-card'","<section class='dashboard-card' id='weekly'")}${section('desserts','dessert').replace("<section class='dashboard-card'","<section class='dashboard-card' id='desserts'")}${drinksSection().replace("<section class='dashboard-card'>","<section class='dashboard-card' id='drinks'>")}</div></div></div><div class='savebar'><span id='msg' class='save-status' aria-live='polite'></span><button class='btn' id='cancel'>Prekliči</button><button class='btn' id='save'>Shrani spremembe</button></div>`;bind();updateSaveStatus();}
