...
function bind(){ document.querySelectorAll('.nav-link').forEach((b)=>b.onclick=()=>{setActivePanel(b.dataset.panel);render();}); const mobileNav=document.getElementById('mobileNav'); if(mobileNav){mobileNav.value=activePanel; mobileNav.onchange=()=>{setActivePanel(mobileNav.value);render();};}
 ...
 const dnevnikForm=document.getElementById('dnevnikForm'); if(dnevnikForm){if(dnevnikEditingId){const activeEntry=(data.dailyEntries||[]).find((x)=>x.id===dnevnikEditingId);if(activeEntry){dnevnikForm.date.value=activeEntry.date||'';dnevnikForm.dishName.value=activeEntry.dishName||'';dnevnikForm.preparedQty.value=activeEntry.preparedQty??0;dnevnikForm.soldQty.value=activeEntry.soldQty??0;dnevnikForm.leftoverQty.value=activeEntry.leftoverQty??0;dnevnikForm.resultStatus.value=activeEntry.resultStatus||'okej';dnevnikForm.notes.value=activeEntry.notes||'';}}const upsertDnevnikEntry=(event)=>{if(event){event.preventDefault();event.stopPropagation();}if(!dnevnikForm.reportValidity())return;snapshotData();const entryId=String(dnevnikForm.entryId?.value||'').trim();const next={id:entryId||crypto.randomUUID(),date:String(dnevnikForm.date?.value||''),dishName:String(dnevnikForm.dishName?.value||''),preparedQty:Number(dnevnikForm.preparedQty?.value||0),soldQty:Number(dnevnikForm.soldQty?.value||0),leftoverQty:Number(dnevnikForm.leftoverQty?.value||0),resultStatus:String(dnevnikForm.resultStatus?.value||'okej'),notes:String(dnevnikForm.notes?.value||'')};const idx=(data.dailyEntries||[]).findIndex((x)=>x.id===next.id);if(idx>=0)data.dailyEntries[idx]=next;else data.dailyEntries.push(next);dnevnikEditingId='';setActivePanel('dnevnik');markDirty();render();};const saveDnevnikEntry=document.getElementById('saveDnevnikEntry');if(saveDnevnikEntry){saveDnevnikEntry.type='button';saveDnevnikEntry.onclick=upsertDnevnikEntry;}}
