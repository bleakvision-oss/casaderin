...
async function save(){
  ...
  updateSaveStatus();
  try {
    data.cateringEntries=Array.isArray(data.cateringEntries)?data.cateringEntries:[]; data.cateringEntries=data.cateringEntries.map((entry)=>({id:entry?.id||crypto.randomUUID(),datum:entry?.datum||'',ura:entry?.ura||'',stranka:entry?.stranka||'',kontakt:entry?.kontakt||'',lokacija:entry?.lokacija||'',stOseb:Number(entry?.stOseb||0),opisNarocila:entry?.opisNarocila||'',dogovorjenaCena:entry?.dogovorjenaCena||'',status:cateringStatusOptions.includes(entry?.status)?entry.status:'povpraševanje',racunIzstavljen:!!entry?.racunIzstavljen,racunPlacan:!!entry?.racunPlacan,opombe:entry?.opombe||''}));
