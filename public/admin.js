...
function bind() {
  document.querySelectorAll('.admin-item').forEach((el) => {
    ...
    el.querySelectorAll('input').forEach((inp) => inp.addEventListener('input', () => { markDirty(); const { f, l, a } = inp.dataset; if (a) { target.allergens = allergenIds().filter((x) => el.querySelector(`[data-a="${x}"]`)?.checked); return; } if (inp.type === 'checkbox') { target[f] = inp.checked; return; } if (l) { target[f] = target[f] || {}; target[f][l] = inp.value; if (f === 'name' && l === 'sl') target._suggestText = inp.value; if (f === 'description' && l === 'sl' && !target._suggestText) target._suggestText = inp.value; render(); } else target[f] = f === 'sortOrder' ? Number(inp.value || 0) : inp.value; }));
