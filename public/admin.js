...
function bindCateringi(){const cateringForm=document.getElementById('cateringForm'); if(cateringForm){if(cateringEditingId){const activeEntry=(data.cateringEntries||[]).find((x)=>x.id===cateringEditingId);if(activeEntry){Object.entries(activeEntry).forEach(([k,v])=>{if(!(k in cateringForm)) return; if(cateringForm[k].type==='checkbox') cateringForm[k].checked=!!v; else cateringForm[k].value=v??'';});}}const saveCateringEntry=document.getElementById('saveCateringEntry'); if(saveCateringEntry){saveCateringEntry.onclick=(e)=>{e.preventDefault();if(!cateringForm.reportValidity())return;snapshotData();const next={id:String(cateringEditingId||'').trim()||crypto.randomUUID(),datum:String(cateringForm.datum?.value||''),ura:String(cateringForm.ura?.value||''),stranka:String(cateringForm.stranka?.value||''),kontakt:String(cateringForm.kontakt?.value||''),lokacija:String(cateringForm.lokacija?.value||''),stOseb:Number(cateringForm.stOseb?.value||0),opisNarocila:String(cateringForm.opisNarocila?.value||''),dogovorjenaCena:String(cateringForm.dogovorjenaCena?.value||''),status:cateringStatusOptions.includes(cateringForm.status?.value)?cateringForm.status.value:'povpraševanje',racunIzstavljen:!!cateringForm.racunIzstavljen?.checked,racunPlacan:!!cateringForm.racunPlacan?.checked,opombe:String(cateringForm.opombe?.value||'')};const entries=data.cateringEntries||[];if(cateringEditingId){const idx=entries.findIndex((x)=>x.id===next.id);if(idx>=0)entries[idx]=next;else entries.push(next);}else entries.push(next);data.cateringEntries=entries;cateringEditingId='';markDirty();render();};}}
 ...
 document.querySelectorAll('[data-delete-catering-id]').forEach((b)=>b.onclick=()=>{snapshotData();const id=String(b.dataset.deleteCateringId||'');data.cateringEntries=(data.cateringEntries||[]).filter((x)=>x.id!==id);if(cateringEditingId===id)cateringEditingId='';markDirty();render();});
