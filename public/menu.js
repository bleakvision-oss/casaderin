...
function introSection(){const updated=formatUpdatedAt(state.data.updatedAt);const st=state.data.settings||{};const hero=st.heroImageUrl?`<figure class='hero-media hero-image'><img src='${st.heroImageUrl}' alt='${st.restaurantName||'Casa de Rin'}' loading='eager' onerror="this.closest('figure').style.display='none'"></figure>`:'';return `<section class='hero-intro'>${updated?`<p class='updated'>${updated}</p>`:''}${settingsSection()}${hero}</section>`;}
