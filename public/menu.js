...
function render(){const d=state.data;const c=document.getElementById('content');const s=d.sections;c.innerHTML=[card(ui('section_daily'),s.daily.map(item)),card(ui('lunch'),s.lunch.map(item)),card(ui('section_weekly'),s.weekly.map(item)),card(ui('dessert'),s.desserts.map(item)),card(ui('beverage'),s.drinks.map(item)),settingsSection()].join('');}
