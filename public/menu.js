...
function drinksSection(){const categories=state.data.drinkCategories||[];const fallback=state.data.sections.drinks||[];if(!categories.length)return card(ui('beverage'),fallback.map((x)=>item(x,{showStatus:false})).join(''));return card(ui('beverage'),categories.map((cat)=>{const rows=(cat.items||[]).filter((x)=>!(x.status==='hidden'||x.hidden)).map(drinkRow).join('');if(!rows)return'';return `<div class='drink-group'><h4>${t(cat.title)}</h4>${rows}</div>`;}).join(''));}
